...
funcSched(cin *cinema)
{
	...
	for(i=0;i<6;i++)
	{
		...
		for(j=0;j<6;j++)
		{
			...
			if((cinema[i].show[j].showstart.hour>=9&&cinema[i].show[j].showstart.half[0]=='p'&&cinema[i].show[j].showstart.hour!=12)||(cinema[i].show[j].showstart.half[0]=='n'))
			{
